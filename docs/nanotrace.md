...
Their slices describe flat envelopes of hardware-timed graph nodes, separate
from the kernel event hierarchy. A range may span asynchronous branches, so
its duration is elapsed time, not summed kernel execution time. The
`range_instance` argument distinguishes disjoint scopes with the same label.

CPU slices can carry a string `semantic_range` argument. It describes an
overlay across the source thread's worker group, as defined by track parent
IDs. These annotations do not replace event names, timings, or ownership.

See the [capture API](../nanotrace-cuda/README.md#graph-regions) for graph
registration and the [viewer guide](../visualizer/README.md) for display behavior.
